...
//namespace popcon {
CTPPSOpticsParamRootFileSourceHandler::CTPPSOpticsParamRootFileSourceHandler(const edm::ParameterSet & pset):
  m_name(pset.getUntrackedParameter<std::string>("name","CTPPSOpticsParamRootFileSourceHandler")),
...
CTPPSOpticsParamRootFileSourceHandler::~CTPPSOpticsParamRootFileSourceHandler() {}
  
void CTPPSOpticsParamRootFileSourceHandler::getNewObjects() {
  edm::LogInfo("CTPPSOpticsParamRootFileSourceHandler") << "[CTPPSOpticsParamRootFileSourceHandler::getNewObjects] for PopCon application " << m_name;
  if(m_debugMode){
    std::stringstream ss;
    ss << "\n------- " << m_name 
       << " - > getNewObjects\n";
    ...
    edm::LogInfo("CTPPSOpticsParamRootFileSourceHandler") << ss.str();
  }
  edm::LogInfo("CTPPSOpticsParamRootFileSourceHandler") << "runnumber/first since = " << m_since << std::endl;
  if(m_since<=this->tagInfo().lastInterval.first){
    edm::LogInfo("CTPPSOpticsParamRootFileSourceHandler") 
      << "[CTPPSOpticsParamRootFileSourceHandler::getNewObjects] \nthe current starting iov " << m_since
  ...
  edm::LogInfo("CTPPSOpticsParamRootFileSourceHandler") 
    << "[CTPPSOpticsParamRootFileSourceHandler::getNewObjects] " << m_name << " getting data to be transferred "  << std::endl;
  FileBlob* rootFile = new FileBlob(m_file,m_zip);
  if(rootFile->size() != 0){
    edm::LogInfo("CTPPSOpticsParamRootFileSourceHandler") << "setting runnumber/first since = " << m_since << std::endl;
    this->m_to_transfer.push_back(std::make_pair(rootFile,m_since));
  } else {
    edm::LogError("CTPPSSummarySourceHandler") << "Root file " << m_file << " does not exist" << std::endl;
  }
  edm::LogInfo("CTPPSSummarySourceHandler") << "------- " 
					    << m_name << " - > getNewObjects" 
					    << std::endl;
}
  
std::string CTPPSOpticsParamRootFileSourceHandler::id() const {return m_name;}
//}
