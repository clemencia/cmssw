/****************************************************************************
 *
 * This is for CT-PPS offline database, based on TOTEM offline SW.
 * Authors:
 *	Jan Kaspar (jan.kaspar@gmail.com)
 *      Clemencia Mora (clemencia.mora@gmail.com)
 *
 ****************************************************************************/
 ...
 * ROOT::Math::RotationZYX(r_z, r_y, r_x), which corresponds to:
  ...
  \endverbatim
 **/
class CTPPSAlignmentCorrection{
public:
  /// full constructor, shifts in mm, rotations in rad
  CTPPSAlignmentCorrection(double sh_r, double sh_r_e, double sh_x, double sh_x_e, double sh_y, double sh_y_e,
  ...
  CTPPSAlignmentCorrection(double sh_x = 0., double sh_y = 0., double sh_z = 0., double rot_z = 0.);

  //destructor
  virtual ~CTPPSAlignmentCorrection(){}
  ...
  void Add(const CTPPSAlignmentCorrection&, bool sumErrors = true, bool addShR=true,
  ...
  // should we get rid of these 2 (Print and WriteXML) for conditions object?
...
protected:
  ...
  DDTranslation translation  COND_TRANSIENT;
  
  /// the uncertainty of shift in mm (if known)  
  DDTranslation translation_error COND_TRANSIENT;
