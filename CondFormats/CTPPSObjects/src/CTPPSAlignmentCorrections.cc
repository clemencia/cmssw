...
void CTPPSAlignmentCorrections::LoadXMLFile(const string &fileName)
{
  printf(">> CTPPSAlignmentCorrections::LoadXMLFile(%s)\n", fileName.c_str());
  ...
  try {
    ...
    throw cms::Exception("CTPPSAlignmentCorrections") << "An XMLException caught with message: " << message << ".\n";
  ...
  try {
    ...
    throw cms::Exception("CTPPSAlignmentCorrections") << "Cannot parse file `" << fn << "' (exception)." << endl;
  }

  if (!parser)
    throw cms::Exception("CTPPSAlignmentCorrections") << "Cannot parse file `" << fn << "' (parser = NULL)." << endl;
  
  DOMDocument* xmlDoc = parser->getDocument();

  if (!xmlDoc)
    throw cms::Exception("CTPPSAlignmentCorrections") << "Cannot parse file `" << fn << "' (xmlDoc = NULL)." << endl;

  DOMElement* elementRoot = xmlDoc->getDocumentElement();
  if (!elementRoot)
    throw cms::Exception("CTPPSAlignmentCorrections") << "File `" << fn << "' is empty." << endl;

  LoadXMLBlock(elementRoot);
...
void CTPPSAlignmentCorrections::LoadXMLBlock(DOMNode *root)
{
  ...
  for (unsigned int i = 0; i < children->getLength(); i++) {
    ...
    if (!nodeType)
      throw cms::Exception("CTPPSAlignmentCorrections") << "Unknown node `" << XMLString::transcode(n->getNodeName()) << "'.";

    // check children
    if (n->getChildNodes()->getLength() > 0)
        edm::LogProblem("CTPPSAlignmentCorrections") << ">> CTPPSAlignmentCorrections::LoadXMLFile > Warning: tag `" <<
                    ...
                    sh_z = atof(XMLString::transcode(a->getNodeValue()));
                      ...
                      sh_z_e = atof(XMLString::transcode(a->getNodeValue()));
                        ...
                        rot_z = atof(XMLString::transcode(a->getNodeValue()));
                          ...
                          edm::LogProblem("CTPPSAlignmentCorrections") << ">> CTPPSAlignmentCorrections::LoadXMLFile > Warning: unknown attribute `"
                            << XMLString::transcode(a->getNodeName()) << "'.";
    }

    // id must be set
    if (!idSet)
        throw cms::Exception("CTPPSAlignmentCorrections") << "Id not set for tag `" << XMLString::transcode(n->getNodeName()) << "'.";

    // build alignment
    CTPPSAlignmentCorrection a(sh_r*1E-3, sh_r_e*1E-3, sh_x*1E-3, sh_x_e*1E-3, sh_y*1E-3, sh_y_e*1E-3,
...
CTPPSAlignmentCorrection& CTPPSAlignmentCorrections::GetRPCorrection(unsigned int id)
{
  return rps[id];
}
...
CTPPSAlignmentCorrection CTPPSAlignmentCorrections::GetRPCorrection(unsigned int id) const
{
  CTPPSAlignmentCorrection a;
...
CTPPSAlignmentCorrection& CTPPSAlignmentCorrections::GetSensorCorrection(unsigned int id)
{
  return sensors[id];
}

CTPPSAlignmentCorrection CTPPSAlignmentCorrections::GetSensorCorrection(unsigned int id) const
{
  CTPPSAlignmentCorrection a;
...
CTPPSAlignmentCorrection CTPPSAlignmentCorrections::GetFullSensorCorrection(unsigned int id,
  bool useRPErrors) const
{
  CTPPSAlignmentCorrection c;
...
void CTPPSAlignmentCorrections::SetRPCorrection(unsigned int id, const CTPPSAlignmentCorrection& ac)
{
  rps[id] = ac;
}
...
void CTPPSAlignmentCorrections::SetSensorCorrection(unsigned int id, const CTPPSAlignmentCorrection& ac)
{
  sensors[id] = ac;
}
...
void CTPPSAlignmentCorrections::AddRPCorrection(unsigned int id, const CTPPSAlignmentCorrection &a,
...
void CTPPSAlignmentCorrections::AddSensorCorrection(unsigned int id, const CTPPSAlignmentCorrection &a,
  bool sumErrors, bool addShR, bool addShZ, bool addRotZ)
{
...
void CTPPSAlignmentCorrections::AddCorrections(const CTPPSAlignmentCorrections &nac, bool sumErrors,
...
void CTPPSAlignmentCorrections::Clear()
{
  rps.clear();
  sensors.clear();
}
...
void CTPPSAlignmentCorrections::WriteXMLFile(const string &fileName, bool precise, bool wrErrors, bool wrSh_r,
  bool wrSh_xy, bool wrSh_z, bool wrRot_z) const
{
  FILE *rf = fopen(fileName.c_str(), "w");
  if (!rf)
    throw cms::Exception("CTPPSAlignmentCorrections::WriteXMLFile") << "Cannot open file `" << fileName
      << "' to save alignments." << endl;

  fprintf(rf, "<!--\nShifts in um, rotations in mrad.\n\nFor more details see CTPPSAlignmentCorrections::LoadXMLFile in\n");
  fprintf(rf, "TotemAlignment/RPDataFormats/src/CTPPSAlignmentCorrectionsSequence.cc\n-->\n\n");
  fprintf(rf, "<xml DocumentType=\"AlignmentDescription\">\n");

  WriteXMLBlock(rf, precise, wrErrors, wrSh_r, wrSh_xy, wrSh_z, wrRot_z);
...
void CTPPSAlignmentCorrections::WriteXMLBlock(FILE *rf, bool precise, bool wrErrors, bool wrSh_r,
  bool wrSh_xy, bool wrSh_z, bool wrRot_z) const
{
  bool firstRP = true;
...
//  **/
// void CTPPSAlignmentCorrections::FactorRPFromSensorCorrections(CTPPSAlignmentCorrections &expanded,
//   CTPPSAlignmentCorrections &factored, const AlignmentGeometry &geometry, bool equalWeights,
...
//     origAlignments[it->first] = GetFullSensorCorrection(it->first, false);
...
//       AlignmentGeometry::const_iterator git = geometry.find(*dit);
//       const DetGeometry &d = git->second;
//       const CTPPSAlignmentCorrection &oa = origAlignments[*dit];
...
//     }
//     catch (...) {
//       printf("ERROR in CTPPSAlignmentCorrections::FactorRPFromSensorCorrections > AT A matrix is singular, skipping RP %u.\n", rpId);
//       continue;
//     }
...
//     factored.rps[rpId] = CTPPSAlignmentCorrection(0., 0., g0x, g0x_error, g0y, g0y_error, 0., 0., rot_z_mean, rot_z_mean_error); 
...
void CTPPSAlignmentCorrections::insertValues(const std::string& identifier, const std::vector<double>& values)
{
  unsigned int key = 0;
  if (sscanf(identifier.c_str(), "%u", &key) < 0)
    throw cms::Exception ("CTPPSAlignmentCorrections::insertValues") << "Invalid identifier `" << 
      identifier << "'." << std::endl;
...
//TYPELOOKUP_DATA_REG (CTPPSAlignmentCorrections);
